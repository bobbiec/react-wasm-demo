...
(Then over a year later, I realized there's a better way to do it entirely by using the `--pre-js` option to read the WASM file out of the `public` folder, which serves the file directly.
...
This approach can also be used for the `.data` files generated by `--preload-file` for the WebAssembly virtual filesystem.
You'll want to check the generated `.mjs` for the logic around `REMOTE_PACKAGE_NAME` to see if the right path is being fetched.
